...
/// half. Callers hold both — an `Item` carries `session_id` and `id` — so the
/// prefix is stripped rather than trusted to be absent, because handing
/// `claude --resume claude:abc` to the CLI fails with a message about a
/// missing conversation and nothing on screen explains the colon.
fn native_session_id<'a>(agent: &str, session_id: &'a str) -> &'a str {
    session_id.strip_prefix(format!("{agent}:").as_str()).unwrap_or(session_id)
}

/// Resume this conversation with borrowed flags attached to that one run.
fn resume_with_flags(agent: &str, session_id: &str, flags: &[String]) -> Result<String, String> {
    let id = native_session_id(agent, session_id);
    if id.is_empty() {
        return Err("that conversation has no native id to resume".into());
    }
    // Quoted like `fork_cmd` does. A native id is normally a uuid, but it is
    // read out of an agent's own file and goes onto a shell prompt, so it is
    // not this function's place to assume it holds no space.
    let mut cmd = native_resume(agent, &shq(id))
        .ok_or_else(|| format!("don't know how to resume a {agent} conversation"))?;
    for flag in flags {
        cmd.push(' ');
        // Quoted as one word each. claude's `--mcp-config={json}` is a single
        // argument containing braces and quotes; split by the shell it becomes
        // several files claude cannot find.
        cmd.push_str(&shq(flag));
    }
    Ok(cmd)
}

/// Resume a conversation with a Skill its Agent does not own.
///
/// Borrowing is per *run*, and a resume is a run, so `lend`'s flags apply
/// unchanged — which is the reason to reuse them rather than write a second
/// set: the `--plugin-dir` shim and the sensitive-source rules are already
/// decided there, in one place.
///
/// The refusal is the part that matters. Three of the eight agent/capability
/// pairings have no one-run syntax at all, and a command assembled for one of
/// those would look right on the prompt and fail after the launcher had closed
/// — so it is an explicit Err here, and the surfaces wave asks
/// `lend::can_borrow_skill` before offering the row at all.
pub fn resume_with_skill_cmd(
    agent: &str,
    session_id: &str,
    skill_dir: &Path,
    skill_name: &str,
) -> Result<String, String> {
    if !crate::lend::can_borrow_skill(agent) {
        return Err(format!(
            "{agent} has no way to load a skill it does not own for one run — \
             install a copy instead"
        ));
    }
    let flags = crate::lend::skill_flags(agent, skill_dir, skill_name)?;
    resume_with_flags(agent, session_id, &flags)
}

/// Resume a conversation with an MCP server its Agent does not own.
///
/// `lend::resolve` asks the owning CLI again on this explicit keystroke: a
/// complete definition — command arguments, env, headers, URL — is never kept
/// in an Item or a cache, so there is nothing here to read it out of. What
/// comes back goes through `lend::mcp_flags`, which is where the two rules
/// that have already cost a bug live: `--mcp-config=` never takes a space, and
/// a server carrying an env block is written to a 0600 file for claude and
/// refused for codex rather than inlined into shell history.
pub fn resume_with_mcp_cmd(agent: &str, session_id: &str, server: &Item) -> Result<String, String> {
    if !crate::lend::can_borrow_mcp(agent) {
        return Err(format!("{agent} cannot take an MCP server for one run"));
    }
    let definition = crate::lend::resolve(server)?;
    let flags = crate::lend::mcp_flags(agent, &definition)?;
    resume_with_flags(agent, session_id, &flags)
}
...
mod tests {
    ...
    #[test]
    fn a_resume_borrows_a_capability_or_says_it_cannot() {
        let dir = scratch("resume-skill");
        // pi takes a skill directory directly, so nothing is staged and this
        // test writes nowhere. claude's route is deliberately not exercised
        // here: `skill_flags` builds its plugin shim inside the real Prelude
        // cache, and a test has no business writing there.
        let cmd = resume_with_skill_cmd("pi", "pi:abc", &dir, "review").unwrap();
        assert_eq!(cmd, format!("pi --session abc --skill {}", shq(&dir.to_string_lossy())));
        // The stable id and the native id are both accepted; only the native
        // half may reach the flag.
        assert_eq!(resume_with_skill_cmd("pi", "abc", &dir, "review").unwrap(), cmd);
        for agent in ["codex", "opencode"] {
            assert!(
                resume_with_skill_cmd(agent, "x:abc", &dir, "review").is_err(),
                "{agent} has no one-run skill flag and must be told so before the launcher closes",
            );
        }

        let plain = crate::lend::Mcp::Stdio {
            name: "notes".into(),
            command: "node".into(),
            args: vec!["server.js".into()],
            env: Vec::new(),
        };
        let with_key = crate::lend::Mcp::Stdio {
            name: "notes".into(),
            command: "node".into(),
            args: vec!["server.js".into()],
            env: vec![("API_KEY".into(), "sk-0123456789abcdefghijklmno".into())],
        };
        let row = |m: &crate::lend::Mcp| {
            Item::new("notes", Kind::Mcp)
                .put("agent", "claude")
                .put("name", "notes")
                // `def` is what `lend::resolve` reads before it would shell
                // out; a test must not start an Agent CLI.
                .put("def", serde_json::to_string(m).unwrap())
        };

        let claude = resume_with_mcp_cmd("claude", "claude:abc", &row(&plain)).unwrap();
        assert!(claude.starts_with("claude --resume abc "), "{claude}");
        assert!(claude.contains("--mcp-config={"), "{claude}");
        assert!(
            !claude.contains("--mcp-config "),
            "--mcp-config is variadic: with a space it swallows the next word",
        );
        let codex = resume_with_mcp_cmd("codex", "codex:abc", &row(&plain)).unwrap();
        assert!(codex.starts_with("codex resume abc "), "{codex}");
        assert!(codex.contains("mcp_servers.notes.command"), "{codex}");
        assert!(
            resume_with_mcp_cmd("codex", "codex:abc", &row(&with_key)).is_err(),
            "codex can only take a server inline, so an env block would land in shell history",
        );
        assert!(resume_with_mcp_cmd("pi", "pi:abc", &row(&plain)).is_err());
        let _ = std::fs::remove_dir_all(dir);
    }
